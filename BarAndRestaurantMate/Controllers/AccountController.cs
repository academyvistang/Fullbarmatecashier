...
using System.Net;
    ...
    namespace BarAndRestaurantMate.Controllers
    {
        ...
        private int GetMode()
        {
            ...
            try
            {
                payAsYouGo = int.Parse(ConfigurationManager.AppSettings["IsRestaurant"].ToString());
            }
            catch
            {
                payAsYouGo = 0;
        ...
        public string GetVisitorIPAddress(bool GetLan = false)
            {
                ...
                string visitorIPAddress = this.HttpContext.Request.ServerVariables["HTTP_X_FORWARDED_FOR"];

                if (String.IsNullOrEmpty(visitorIPAddress))
                    visitorIPAddress = this.HttpContext.Request.ServerVariables["REMOTE_ADDR"];

                if (string.IsNullOrEmpty(visitorIPAddress))
                    visitorIPAddress = this.HttpContext.Request.UserHostAddress;
            ...
            public ActionResult SelfService()
            {
                ...
                return View(new BaseViewModel { IsPresent = isPresent, FullGuestName = fullGuestName, UserName = userName, Password = password });
            }
            ...
            public ActionResult NewLoginGuest()
            {
                ...
                if (mode == 1)
                {
                    return View("NewLoginGuestRestaurant", new BaseViewModel());
                }
                else
                {
                    return View(new BaseViewModel());
            ...
            public ActionResult NewLogin()
            {
                ...
                {
                    return View("NewLoginGuest", new BaseViewModel());
                }
            }
