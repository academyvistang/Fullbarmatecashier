        ...
        public ActionResult FutureReservationNewBooking(string selectedRoomIds, RoomBookingViewModel model, int? paymentMethodId, string paymentMethodNote)
        {
            ...
            if (roomIds.Any())
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    //return RedirectToAction("EditFutureBooking", "Booking", new { id = guest.Id });
            ...
            if (ModelState.IsValid && commitChanges)
            {
                ...
                if (!conCurencyIssue)
                {
                    ...
                    if(model.TaxiAmount > 0)
                    {
                        ...
                        //_personService.GetAllForLogin().FirstOrDefault(x => x.Username.ToUpper().Equals(User.Identity.Name.ToUpper())).PersonID
                        em.StaffId = _personService.GetAllForLogin().FirstOrDefault(x => x.Username.ToUpper().Equals(User.Identity.Name.ToUpper())).PersonID;
                    ...
                    if(model.InitialDeposit > 0)
                    {
                        strTextMessage = "You have a new room booking, Room No. is " + newRoom.RoomNumber + ". An initial depoit was paid. NGN" + model.InitialDeposit + ", Payment Method is : "  + paymentMethodNote + ". Have a good day sir.";
                    }
                    else
                    {
                        strTextMessage = "You have a new room booking, Room No. is " + newRoom.RoomNumber + ". No initial depoit was paid";
                    }   

                    SendSMS("", "", strTextMessage);
                }
                else
                {
